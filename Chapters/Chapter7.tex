...
\end{align}
is expressed in terms of two conjugate variables $q$ and $d/dq$ with the help of a special representation of the angular momentum components. Indeed, by using the Jacobi Elliptic functions \cite{jacobi1829fundamenta,akhiezer1990elements} $\text{sn}$ ($s$), $\text{cn}$ ($c$), and $\text{dn}$ ($d$), respectively. These functions are used quite often in physics, especially in non-linear problems \cite{kovacic2016jacobi}, with their expressions:
\begin{align}
    s&\equiv\text{sn}(q,k)\ ,\ c\equiv\text{cn}(q,k)\ ,\ d\equiv\text{dn}(q,k)\ ,\nonumber\\
    k=\sqrt{|u|}\ ,\ k'&=\sqrt{1-k^2}\ ,\ q=\int_0^\varphi \frac{1}{\sqrt{1-k^2\sin^2(x)}}dx\equiv F(\varphi,k)\ .
\end{align}