...
Throughout Chapter \ref{extra-chapter-new-boson} a rather unique approach to the wobbling effect in triaxial nuclei was presented. It started with the Hamiltonian specific for an even-even core that interacts with a single-particle \cite{raduta2017semiclassical,raduta2018wobbling,raduta2020approach}. The coupling term, which describes the motion of the odd nucleon in a deformed mean field generated by the core is a priori ignored, since a rigid coupling is employed. Such a property will not lead to any modifications in the energy spectrum, thus it can be safely dismissed. 

By using the Bargmann mapping for the angular momentum components, it is possible to express the Hamiltonian in terms of boson operators. The Hamiltonian is further  written this boson space by doing an \emph{elliptic expansion} on the angular momentum (recall Eq. \ref{boson-hamiltonian-complete-form}). This is done via the Jacobi elliptic functions, given in Eq. \ref{elliptic-functions-equation}. Usually, these are used for describing complex but periodic systems because they consist of generalized trigonometric functions. As a matter of fact the rotation of the triaxial nucleus itself can be seen as a \emph{periodic motion}, with the wobbling frequency directly emphasizing such character.

Through the adoption of the elliptic functions and the Bargmann representation, an expression for the triaxial potential is acquired (i.e., Eq. \ref{elliptic-potential-formula}). From the eigenvalue equation for the Hamiltonian, a Schrödinger-like equation is reached, with a fully separated kinetic and potential term.

Energia potentiala are 4 minime  simetrice unele (2) fata de altele (2). Procedura permite o cuantificare a energiei potentiale, diferita de  cea prescrisa de Pauli in anul 1933. Prezenta mai multor minime este specifica functiilor eliptice. Numai aceasta reprezentare permite stabilirea unei legaturi intre miscarea de wobbling si cea chirala. Evident functiile eliptice nu se pot folosi pentru descrierea fenomenelor neperiodice. Amanunte despre functiile eliptice poti gasi in Wikipedia cerand pe google "functii eliptice". Te rog confirma primirea acestui e-mail.