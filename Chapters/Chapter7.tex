...
In the case of this formalism, the parity transformation (parity operator) is defined as the product between the complex conjugation operator and a rotation of angle $\pi$ around the quantization axis. Moreover, the total parity is defined as the product between the parity operator of the core and the single-particle one:
\begin{align}
    \hat{P}_T=\hat{P}_\text{core}\otimes\hat{P}_\text{sp}\ .
    \label{parity-operator-aw2}
\end{align}

When acting with the parity operator defined in Eq. \ref{parity-operator-aw2} on the trial function, the change of variables will be affected in the following manner:
\begin{align}
...
The remarking feature of the last two equations is that the wave-function that describes the triaxial system and its image through the action of $\hat{P}_T$ form a set of two linearly dependent functions which only differ by a multiplicative constant $p$, with $|p|=1$. Consequently, $p$ can either be $+1$ or $-1$, such that:
\begin{align}
    \bar{\Psi}(r,\varphi;f,\psi)=\pm\Psi(r,\varphi;f,\psi)
\end{align}

This result is very important here because it reflects the fact that the triaxial rotor described with the initial Hamiltonian admits eigenfunctions of both negative and positive parity. As such, a single wave-function that is characterized by the coupling of a triaxial core to a valence nucleon can indeed have wobbling bands with negative parity. For the case of $\Psi(r,\varphi;f,\psi)$ and its Hamiltonian describing $^{163}$Lu, \emph{the fourth band TSD4 can emerge from the coupling of the same odd proton as the other three bands}. Concluding the analysis within the $\mathbf{W_2}$ formalism, the bands TSD2 and TSD4 can be indeed considered parity partners, meaning that they are both generated by the coupling of a triaxial even-even core with an identical quasi-particle.

The fact that the positive partner TSD2 has states that lie lower in energy than its negative partner is a rather interesting characteristic of the present model. New experimental data on wobbling nuclei that comprise both positive and negative parity bands could encourage pursuing a research on the relative spacing between the partners.
