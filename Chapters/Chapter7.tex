...
Proceeding with the workflow illustrated in Fig. \ref{fitting-workflow-fig}, a numerical implementation is obtained. Results and discussions will be presented in this section. Because $^{163}$Lu is the only isotope to have positive and negative parity bands in the same collective structure, $\mathbf{W_2}$ will be tested on this nucleus. In what follows, the numerical results concerning the spectrum of excitation energies will be presented. The excitation energies are defined as per the rule from Eq. \ref{excitation-energy-general-formula}, and their values are used in the minimization of the $\chi^2$-function (Eq. \ref{chi-2-fitting-function}). Firstly, the normalization adopted in $\mathbf{W_2}$ is sketched in Table \ref{lu163-table-info} and with the information from this table, the minimization procedure of $\chi^2$-function can be performed. 
\begin{table}
    \centering
