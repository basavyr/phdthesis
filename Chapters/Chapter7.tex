...
The expression of $E_n$ is then used to evaluate the numerical values for the \emph{excitation energies} of $^{135}$Pr. This isotope has three wobbling bands, and the fitting procedure adopted in the current model describes them very well, providing similar results with other existing models within literature. The results concerning excitation energies (Eq. \ref{excitation-energies-new-boson}) were shown in Figs. \ref{135pr-new-boson-band1} - \ref{135pr-new-boson-band23}. Section \ref{numerical-results-new-boson} shows numerical results also for the electromagnetic transition probabilities. Good agreement with the experimental data is obtained as well in this formalism.

Lastly, in Section \ref{new-boson-chiral-character}, a special analysis is exploited for the initial Hamiltonian $\hat{H}_\text{rot}$. The two unique fingerprints for triaxiality in nuclei are discussed, namely the wobbling and chiral motions. Based on the intrinsic structure of $\hat{H}_\text{rot}$, both a symmetric and a asymmetric part could be identified, where the symmetric/asymmetric character is reflected by the chiral transformation $\theta=\pi\longrightarrow\theta'=\theta+\pi$. Thus, for the potential $V(q)$, a symmetric and an asymmetric term were composed (see Eq. \ref{chiral-potentials-symmetric-asymmetric}) and their graphical representations show remarkable behavior (see Fig. \ref{chiral-potentials-plot}). The same analysis could be performed for the excitation energy of the yrast band, and two energy bands were analytically obtained: one 
...
%TO-DO: connect the chiral study with the wobbling motion.