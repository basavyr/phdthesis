...
The last part will be focused on the same wobbling phenomenon but with a different approach than the ones from Chapters \ref{chapter-4-aw1-formalism} ($\mathbf{W_1}$) and \ref{chapter-5-novel} ($\mathbf{W_2}$), as this method does not share the same foundational concepts. The results shown here correspond to a publication made by the team in Ref. \cite{raduta2020new}. The structure of the chapter is organized as follows: a set of results achieved by the $\mathbf{W_0}$ formalism are briefly recalled in the first section.
...
The Hamiltonian is written in a boson space by doing an \emph{elliptic expansion} on the angular momentum (recall Eq. \ref{boson-hamiltonian-complete-form}). This is done via the Jacobi elliptic functions, given in Eq. \ref{elliptic-functions-equation}.