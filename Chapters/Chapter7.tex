...
The wobbling frequency for $^{135}$Pr can be easily determined by using the moments of inertia and the angle $\theta$ in its analytical expression (see Eq. \ref{wobbling-frequency-new-boson} for $\omega$ and Eq. \ref{wobbling-frequency-new-boson-prime} for $\omega'$). A set of graphical representations are made in Figs. \ref{wobbling-frequencies-135pr} where each plot is generated by using the numerical values from Table \ref{new-boson-fitting-parameters}, the fit when $\mathcal{I}_2=\mathcal{I}_\text{max}$, and lastly, the fit when the excitation energies are evaluated with $\omega'$.
\begin{figure}
    \centering
