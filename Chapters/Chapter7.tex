...
\end{align}
and for $q=\pm2K$ (that is $\varphi=\pm\pi$) one has $\hat{I}_0=-I$, while $q=0$ (i.e., $\varphi=2$) will lead to $\hat{I}_0=I$. This would mean that for the local minima, the total angular momentum will align anti-parallel with the 1-axis, and for the deep minimum $q=0$, the angular momentum is aligned along the 1-axis. Expanding the elliptic potential $V(q)$ up to second order in the coordinate, an equation that looks similar to the harmonic oscillator for $\hat{H}_\text{rot}$:
\begin{align}
    E_n=A_1I^2+\hbar\omega\left(n+\frac{1}{2}\right)+e_\text{sp}\ ,
    \label{energy-2nd-order-expansion-deepest-minima}
\end{align}
where the single-particle term has the expression:
\begin{align}
...
\begin{align}
    \omega^2=&\left[(2I+1)\left(A_2-A_1-\frac{A_2j_2}{I}\right)+2A_1j_1\right]\cdot\nonumber\\
                &\cdot\left[(2I+1)(A_3-A_1)+2_A1j_1\right]-(A_3-A_1)\left(A_2-A_1-\frac{A_2j_2}{I}\right)\ .
    \label{omega-frequency-deepest-minima}
\end{align}

If, on the other hand, the potential is expanded around the local minimum $q=2K$, and only the quadratic terms are kept, then the following set of energies are obtained:
\begin{align}
    E_n^{(2K)}&=-\frac{(2I+1)^2}{2}\frac{2v_0}{2I+1}+\hbar\omega^{(2K)}\left(n+\frac{1}{2}\right)\ ,\nonumber\\
    \left(\omega^{(2K)}\right)^2&=\left(1+\frac{2v_0}{2I+1}\right)\left(u+\frac{2v_0}{2I+1}\right)(2I+1)^2-u\ ,
\end{align}

The two wobbling frequencies, i.e., $\omega$ and $\omega^{(2K)}$ are both graphically represented in Fig. \ref{wobbling-frequencies-harmonic-approx}.
\begin{figure}
    \centering
    \caption{Wobbling frequency.}
