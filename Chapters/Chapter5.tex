...
In the following discussion, some notations will be employed, in order to avoid repeating certain words. Namely, a quasi-particle with hole character will be referred to as $\mathscr{H}$, a proton/neutron will be labelled as $\mathscr{P}$, and the core will be denoted with $\mathscr{C}$. The stability of the triaxial nuclear shapes consists of existence of minima within the energy function (see discussion on PES given in Section \ref{potential-energy-surfaces-section}). In order to minimize the system's energy, the quai-particle will tend to create a specific coupling with the triaxial core, such that the overlap between the density distribution of the triaxial core and the quasi-particle becomes:
\begin{itemize}
    \item maximal for particle + core (minimizes the attractive short range force between the two)
    \item minimal for hole + core (minimizes the repulsive short range force between the two) 
\end{itemize}