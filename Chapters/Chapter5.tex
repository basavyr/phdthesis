...
\begin{figure}
    ...
    \begin{tikzpicture}[every text node part/.style={align=center}]
        ...
        \node[rectangle, draw=black, thick, fill=green!10, inner sep=0.2cm,minimum width=8 cm] (third-box) [below=of second-box] {\textbf{Stable triaxial shape} \\ $\downarrow$ \\ \textbf{Transverse Wobbling Motion}: \\ $\mathbf{j}_{\mathcal{Q}_p}\perp (m_\text{axis}\ ||\  \mathbf{j}_\mathscr{C})$};
...
\begin{tikzpicture}[every text node part/.style={align=center}]
    ...
    \node[rectangle, draw=black, thick, fill=red!10, inner sep=0.2cm,minimum width=8 cm] (third-box) [below=of second-box] {\textbf{Stable triaxial shape} \\ $\downarrow$ \\ \textbf{Transverse Wobbling Motion}: \\ $\mathbf{j}_{\mathcal{Q}_h}\perp (m_\text{axis}\ ||\  \mathbf{j}_\mathscr{C})$};
...
\begin{figure}
    \centering
    \includegraphics[scale=0.6]{Chapters/Figures/longitudinal_wobbler-schematic.pdf}
    \includegraphics[scale=0.6]{Chapters/Figures/transverse_wobbler-schematic.pdf}
    \caption{Geometrical representation with the alignment scheme for a longitudinal wobbler and a transverse wobbler. The the a.m. vector for the odd-nucleon is represented by $\mathbf{j}$, for the core is represented by $\mathbf{R}$, and the total a.m. is represented by $\mathbf{I}$. Note that the precessional motion with oscillator-like behavior of $\mathbf{I}$ is illustrated with the encircled sine wave. The resulting motion of the total a.m. will consist in a precessional cone that is properly depicted in Figs. \ref{advanced-quasiparticle-coupling-1}-\ref{advanced-quasiparticle-coupling-2}.}
