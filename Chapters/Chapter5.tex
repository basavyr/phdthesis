...
\begin{center}
    ...
    \begin{tikzpicture}[every text node part/.style={align=center}]
        \begin{axis}[scale=0.6,every axis plot post/.append style={
            ...
            grid = major] % extend the axes a bit to the right and top
            
            \addplot [name path=model, color=red,thick] {\gauss{0}{0.5}};
            \addplot [name path=obs, color=blue,very thick] {\gauss{0.5}{0.65}};
            \legend{$\mathcal{Q}_p$,$\mathscr{C}$}
            ...
            \addplot[color=green!50!white] fill between[of= lower and axis];
            \node[rectangle, draw=none, thick, fill=none] at (axis cs: 0.8,0.9) {Density distributions for $\mathcal{Q}_p$ and $\mathscr{C}$};
        \end{axis}
        
        \node[rectangle, draw=black, thick, fill=green!10, inner sep=0.2cm] at (3,-3) (first-box) {\textbf{Minimized} short-range interaction \\ \\ $\mathcal{Q}_p+\mathscr{C}$ = \emph{attractive}};
        
        \node[rectangle, draw=black, thick, fill=red!10, inner sep=0.2cm] (second-box) [below=of first-box] {\textbf{Minimal} energy (PES)};
        
        \node[rectangle, draw=black, thick, fill=blue!10, inner sep=0.2cm] (second-box) [below=of second-box] {\textbf{Stable triaxial shape} \\ $\downarrow$ \\ \textbf{Wobbling Motion}};
    \end{tikzpicture}
