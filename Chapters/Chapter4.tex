...
Solving thus Eq. \ref{energy-surface-correction-terms} will consist in a \emph{qualitative} analysis for the behavior of the nucleonic matter at high spins. This `solutions' will consist in graphical representations in the $\beta,\gamma$-plane, where values with the potential energy itself appear, with different magnitudes. The lower these values the `more stable' the deformation is. Typically, one can find regions of stability across the plane, with well-defined values for $\beta$ and $\gamma$ at which nucleus is stable and (obviously) deformed. On the other hand, if the energy variation is rather flat in the $\gamma$ degree of freedom, but centered around a finite $\beta$, then this nucleus is called \emph{$\gamma$-unstable}. These energy surfaces are a key indicator of stability with respect to the deformation parameters. Some examples of energy surfaces for some nuclei are shown in Figs. It is remarkable the fact that small quantum fluctuations will exist around the minima.
In order to explain the theoretical implications of Figs. , one needs to understand that based on a MHO, a so-called \emph{Ultimate Cranker} (UC) numerical implementation has been developed \cite{bengtsson1989method,bengtsson1990high}. This implementation is able to give the values for $(\beta,\gamma)$ at which a nucleus might achieve stability, practically identifying local and global minima within the potential energy surface. In a future section, multiple isotopes with known stable triaxiality will be classified in terms of the deformation parameters, creating thus an `inventory' with these deformed nuclei.
