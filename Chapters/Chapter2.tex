...
It is possible to summarize the various nuclear shapes that can occur with the help of a diagram done in the $(\beta,\gamma)$ plane. The repeating pattern of the nuclear shapes is graphically represented in the figure below.

\begin{figure}
    \centering
    \includegraphics[scale=0.6]{Chapters/Figures/beta_gamma_plane.png}
    \caption{Beta gamma plane.}
    \label{beta-gamma-plane}
\end{figure}

The $(\beta,\gamma)$ plane shown above, also emphasizes the change in shape of the nuclear surface, for a fixed $\beta_2$, due to the change in the ratio between axes. See Fig. \ref{beta-gamma-plane}.
