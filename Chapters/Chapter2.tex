...
% TO-DO write introduction for the chapter
% TO-DO connect all content from the previous C2, C3, C4 into one intro
In order to understand nuclei in terms of their shape, and to get a grasp on how deformation plays a major role in affecting the dynamics of nuclear matter, it is necessary to introduce some theoretical models and definitions. These will act as \emph{tools} in the development of the formalism that will be presented later on. It is rather intuitive that a key characteristic related to the size and shape of the nucleus is its radius. Therefore, the first section in this chapter aims at giving an insight on the nuclear radius and how it can be approximated, showing that different approximations lead to specific types of deformations. Moreover, a realistic description of the nuclei would require a time-dependent behavior of its surface, i.e., vibrations or surface oscillations. Remarkably, these constant oscillations near the surface give, on average, a stable overall shape.

As an outline of this chapter, the shape and deformation will be detailed in Section \ref{nuclear-shapes}, while Section \ref{section-nuclear-models} will cover the specific models for deformed nuclei. Lastly, Section \ref{section-triaxial-signatures} will introduce the two unique characteristics to nuclei lacking symmetry in regards to their equilibrium shape. A set of conclusions and some key takeaways that emerge from all sections will be inferred at the end of the chapter.
...
In this section, the Hamiltonian for a system composed of one single-particle and a triaxial core, which is usually applied to odd-$A$ nuclei, will be discussed. In fact, the main framework that will be described in the remaining chapters is developed starting from the triaxial PRM \cite{davydov1958rotational}. Davydov et al. developed the triaxial PRM model for explaining the low-lying collective spectra of $2^+$ states within some transitional nuclei. The Hamiltonian of this system is composed of a term for the even-even core and another one for a single-particle \emph{moving in a quadrupole deformed mean-field}:
\begin{align}
