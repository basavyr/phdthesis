...
Evaluating the critical points of $\mathcal{H}$, it is possible to obtain the points at which $\mathcal{H}$ is minimal. The ordering of the three inertia factors $A_{1,2,3}$ will determine some constraints on the generalized coordinates for which such condition is met. Choosing the largest MOI to be around the $3$-axis and $\mathcal{I}_3>\mathcal{I}_2>\mathcal{I}_1$ (or, equivalently $A_1<A_2<A_3$), the function achieves a minimum value at $p_0=(I,0;j,0)$:
\begin{align}
