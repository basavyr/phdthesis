...
\end{align}
The states that are involved in $\Psi_\text{trial}$ are as follow: $\ket{IMK}$ represent the Wigner-D functions (i.e., eigenstates of $\hat{I}^2$ and $\hat{I}_3$), while $\ket{j\Omega}$ are the wave-functions of the odd quasi-particle (the eigenstates of $\hat{j}^2$ and $\hat{j}_3$). Notice that the trial function is expressed explicitly in terms of the \emph{extremal states}, i.e., their maximal projections $K=I$ and $\Omega=j$, respectively. For $\ket{IMK}$, one has:
\begin{align}
