...
All the states from TSD1 are produced by coupling the $j=13/2$ odd proton from $i$-shell with a triaxial even-even core having the spin sequence $\mathscr{C}_1=0,2,4,\dots$ (and positive parity), forming thus a zero-phonon wobbling band. On the other hand, states belonging to TSD2 are built by the same odd-particle but a different \emph{core}, namely $\mathscr{C}_2=1,3,5\dots$, also having positive parity. Notice the even/odd dissimilarity between the two angular momentum sequences of the core $\mathscr{C}_1$ and $\mathscr{C}_2$. The wobbling bands TSD1 and TSD2 from $^{163}$Lu can be explained by means of a semiclassical model, through an even-odd staggering of states $(0^+,1^+)$, $(2^+,3^+)$ of collective nature. Moreover, the negative parity states belonging to the TSD4 band can be explored semi-classically via TDVE with the collective core $\mathscr{C}_1=0,2,4$ coupled to the negative parity proton from $h$-shell having $j=9/2$. 

In terms of quasi-particle + triaxial core couplings, it is remarkable the fact that the `final picture' for $^{163}$Lu is regarded as a positive parity core of even states that generates the band TSD1 and TSD4, an odd core with positive parity states forming TSD2, and finally the one-phonon states in TSD3, which are activated from the ground TSD2. One can encapsulate the renormalization under the following \emph{equalities}:
\begin{align}
