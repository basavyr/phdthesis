...
\end{align}
whose action the trial function defined in Eq. \ref{trial-function-appeoach-w1} will change make the change $(\rho,\varphi;\sigma,\psi)\to(\rho,\varphi+\pi;\sigma,\psi+\pi)$, where the complex variables $z$ and $s$ were expressed in terms of the classical coordinates as per Eq. \ref{z-s-variables}. Because the CEF is invariant to $(\rho,\varphi;\sigma,\psi)\to(\rho,\varphi+\pi;\sigma,\psi+\pi)$, then $\ket{\Psi_{IMj}}$ is also invariant. The eigenvalues of $\mathcal{R}_x(\pi)$ are $r_x(\pi)=e^{-\iu\pi(I+j)}$. In the odd-$A$ nuclei the eigenvalues can be $r_x(\pi)=\pm\iu$, meaning that the signature number is either $+1/2$ or $-1/2$. For the present case, two possible angular momentum sets emerge: $\frac{13}{2},\frac{17}{2},\frac{21}{2},\dots$ and $\frac{15}{2},\frac{19}{2},\frac{23}{2},\dots$, respectively. Indeed, the first set would correspond to the TSD1 band while the second set is identified with TSD2. The two sequences have opposite signature and lead to the $\Delta I=1\hbar$ between the two bands. This shows that the assignment of Signature Partner Bands for TSD1,TSD2 holds in the current formalism, since the wave-function (including its semi-classical description through the variables $\rho,\varphi;\sigma,\psi$) admits states with both positive ($1/2$) and negative ($-1/2$) signature.