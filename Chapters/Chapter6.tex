...
\end{align}
where $M_R$ is the projection of the core's a.m. onto the quantization axis (in the lab system). The quantum number $K$ still signifies the projection of $\mathbf{I}$ onto the quantization axis (in the intrinsic system). Obviously, the term $C_{RK}$ is in line with the factors defined in Eq. \ref{CIK-binomial-coefficient}. Note that this wave-function has a different shape than the one from Eq. \ref{wave-function-TSD1-p0-first-order}, where the expansion was in terms of the state vectors $\ket{IMK}$ and not $\ket{RM_RK}$. Having the wave-functions allows to finally evaluate the m.e. of the magnetic dipole transition operator $B(M1)$. 

The matrix elements are also needed in order to obtain the mixing ratio that shows the \emph{competition} between E2 and M1 transitions from states belonging to TSD2 and TSD1 \cite{krane1970determination,toki1975asymmetric}:
\begin{align}
