...
The two terms $T_{2\mu}^\text{coll}$ and $T_{2\mu}^\text{sp}$ from Eq. \ref{quadrupole-transition-operator-terms} are explicitly given  as \cite{raduta2017semiclassical,raduta2020towards}:
\begin{align}
    \mathcal{M}(E2,\mu)=z_\text{eff}\left[Q_{20}\mathcal{D}_{\mu 0}^2-Q_{22}\left(\mathcal{D}_{\mu 2}^2+\mathcal{D}_{\mu -2}^2\right)\right]+z_\text{eff}\sum_{\nu=-2}^2r^2Y_2^\nu\mathcal{D}_{\mu\nu}\ ,
    \label{electric-quadrupole-transition-operator}
\end{align}

The two quadrupole moments $Q_{20}$ and $Q_{22}$ represent a measure of deformation and asymmetry of the nuclear shape. They are expressed in terms of the deformation parameters $\beta$ and $\gamma$, i.e. \cite{raduta2018wobbling}:
\begin{align}
    Q_{20}=\frac{3}{4\pi}ZR^2\beta\cos\gamma\ ,\ Q_{22}=\frac{3}{4\pi}ZR^2\sin\gamma/\sqrt{2}\ ,
    \label{quadrupole-components-Q0-Q2}
\end{align}
which is in fact consistent with the discussion from Chapter \ref{chapter-3} (see Eq. \ref{quadrupole-moment-Q0}) and Chapter \ref{chapter-5} (recall Eq. \ref{quadrupole-components-q20-q22}). In terms of the nuclear charge density, $Q_{20,22}$ show to which extent its distribution deviates from spherical symmetry \cite{bohr1998nuclear}.

From the calculated parameters $\mathcal{P}_\text{fit}$ shown in Table \ref{numerical-fitting-parameters-Lu-isotopes} it was concluded that the maximal MOI is $\mathcal{I}_1$ for each isotope. Consequently, $Q_{20}$ and $Q_{22}$ are the moments that give the degree of quadrupole deformation (elongation) and asymmetry with respect to this axis. Because the formalism $\mathbf{W_1}$ adopts a set of MOI which are determined freely from Eq. \ref{chi-2-fitting-function}, one cannot infer their actual nature (i.e., hydrodynamical or rigid). Additionally, the textbook example of quadrupole components from Eq. \ref{quadrupole-components-Q0-Q2} emerge from hydrodynamic considerations, and for that reason the two quadrupole moments are also considered as free quantities. Consequently, the analytical expressions from Eq. \ref{quadrupole-components-Q0-Q2} can not be used explicitly within the numerical evaluations. They are determined by fixing the reduced transition probability for one intraband and one interband transition ($\text{TSD2}\to\text{TSD1}$). With this method, one obtains for example $Q_0=18.43\ \text{eb}$ and $Q_2=19.81\ \text{eb}$ for $^{163}$Lu. The results for the quadrupole moment calculations concerning the other isotopes are graphically represented in Fig. \ref{quadrupole-moments-fit-numerical-results}.
\begin{figure}
    \centering
    \includegraphics[scale=0.65]{Chapters/Figures/Lu-exp-energies/fig19.pdf}
    \caption{The calculated quadrupole moments $Q_{0}$ and $Q_2$, which enter in Eq. \ref{electric-quadrupole-transition-operator}. The two values are determined by fixing one intraband and one interband $B(E2 \ ;\ \text{TSD2}\to\text{TSD1})$ reduced probabilities from each nucleus. The unit for $Q$ is $\text{eb}$ and the mass number $A$ corresponds to each nucleus.}
