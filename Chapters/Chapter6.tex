...
Naturally, calculations concerning the transition probabilities for each nucleus will also be provided, making a comparison with the available experimental data. The team's work with the $\mathbf{W_1}$ formalism concluded in two research papers \cite{raduta2020approach,raduta2020towards}, in which the isotopes $^{161,163,165,167}$Lu were studied.

Firstly, the fitting procedure described in the previous section was applied to each mentioned isotope where as a first step, one aimed at calculating the excitation energies from Eq. \ref{tsd-bands-general-spectrum}. The set of energies sketched for $^{163}$Lu in Eq. \ref{lu163-absolute-energies-tsd1234} that gave the wobbling spectrum in the new renormalization can be similarly adopted to the other isotopes if one knows the two wobbling phonon numbers, the quasi-particle + core coupling scheme, and the spin + parity assignments for each experimental wobbling band. The $\mathbf{W_1}$ model interprets bands TSD1 and TSD2 in $^{163}$Lu as being ground-states obtained variationally from the coupling of the same odd particle with two different cores. The remarking feature in this theory is that the same can be applied to the other isotopes: namely the pair $(TSD1,TSD2)$ is regarded as zero-phonon bands that originate from different even-even cores. Some fitting rules should be taken into account for each isotope, such that the numerical implementations will be consistent across all nuclei. Taking into consideration the renormalization from Eqs. \ref{renormalized-bands-structure-TSD124} - \ref{renormalized-bands-structure-TSD3}, these rules are given as follow:
\begin{itemize}
    \item $^{161}$Lu
    \begin{itemize}
        \item There are only two TSD bands, emerging from the coupling of $\mathcal{Q}_1$ with the even-even core
        \item Concerning their nature, both bands are ground-states (zero-phonon)
    ...
    \item $^{165}$Lu
    \begin{itemize}
        \item The three TSD bands are created from a single quasi-particle (i.e., $\mathcal{Q}_1=\pi(i_{13/2})$), which couples to a core of even states (TSD1) and a core with odd states (TSD2).
...
One can summarize the set of characteristics for each isotope mentioned above into a set of tables where, for the sake of completeness, the experimental data concerning spin and parity are indicated. Tables \ref{lu-161-experimental-data-table} - \ref{lu-167-experimental-data-table} contain these values. Note that besides the fourth TSD band from $^{163}$Lu, every isotope has wobbling excitations which emerge from the coupling of $\mathcal{Q}_1$ single-particle with an even-even core (containing either even or odd spin states). Since the previous sections contain the relevant data concerning $^{163}$Lu (recall Tables \ref{lu-163-table-info} and \ref{lu-163-phonon-numbers}), it has been omitted.
...
\begin{table}
    ...
    \hline
    Band & Spins                        & $\mathcal{Q}$ & $\mathscr{C}$         & $(n_{w_1},n_{w_2})$ \\ \hline
    TSD1 & $25/2^+,29/2^+,\dots,89/2^+$ & $j_1=13/2^+$  & $6^+,8^+,10^+\dots$   & $(0,0)$             \\ \hline
    TSD2 & $35/2^+,39/2^+,\dots,91/2^+$ & $j_1=13/2^+$  & $11^+,13^+,15^+\dots$ & $(0,0)$             \\ \hline
    TSD3 & $41/2^+,45/2^+,\dots,81/2^+$ & $j_1=13/2^+$  & $14^+,16^+,18^+\dots$ & $(1,0)$             \\ \hline
    \end{tabular}%
    }
...
Putting together the set of energies from Eq. \ref{lu163-absolute-energies-tsd1234}, a general formula that will illustrate the analytical formalism for the excitation energy used in the fitting procedure of Eq. \ref{chi-2-fitting-function} is given as:
\begin{align}
    E_I^\text{TSDK}(^A\text{Lu;exc})=\left(\epsilon_{j}+\mathcal{H}_\text{min}^I+F^I_{n_{w_1}n_{w_1}}\right)-E_{I_0}^\text{TSD1}(^A\text{Lu;abs})\ ,
\end{align}
where $A$ represents the isotope (that is $\{161,163,165,167\}$).