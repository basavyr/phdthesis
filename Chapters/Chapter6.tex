...
The electromagnetic transitions between states were also calculated with the $\mathbf{W_1}$ formalism and compared with the available experimental data. For collective phenomena, one usually expects enhanced transitions of quadrupole character and also large quadrupole moments. In fact, as it was discussed in Chapters \ref{chapter4} and \ref{chapter-5}, the large quadrupole moments and large mixing ratios $\delta$ comprise crucial `tests' for wobbling behavior. Regarding the types of transitions, the intraband and interband transitions are of interest. A schematic representation that illustrates the difference between interband/intraband transitions can be seen in Fig. \ref{schematic-interband-intraband-E2}.
\begin{figure}
    \centering
    \includegraphics[scale=1]{Chapters/Figures/transitions-wobbling-states.pdf}
    \caption{Schematic representation with two collective bands.}
