...
\begin{align}
    ...
    T=\frac{1}{j(j+1)}\sqrt{3}\sin\left(\sqrt{3}\cos\gamma+\sin\gamma\right)\ &,\ T'=\frac{1}{j(j+1)}2\sqrt{3}\sin\gamma\ .
\end{align}
...
\begin{figure}
    \centering
    \includegraphics[scale=0.75]{Chapters/Figures/omega-1-2-frequencies-edited.pdf}
    \caption{The oscillator frequencies $\omega_1$ and $\omega_2$ from Eq \ref{small-omega-1-2}. Calculations are given for arbitrary values of the single particle potential $V$, triaxiality parameter $\gamma$ and single-particle $j$-shell. For this particular example, $j=13/2$. The region with $I\leq j$ is only for illustrative purpose, since in a realistic representation the total spin of the nucleus will always be larger than $j$. The inertia factor ordering is given as $A_3>A_2>A_1$, according to the conditions of existence for $\omega_1$ and $\omega_2$.}
    \label{fig-omega1-omega2}
\end{figure}

In Fig. \ref{fig-omega2-V-Gamma} a different approach is made for $\omega_2$. Indeed, since its dependence is given by $\gamma$ and also $V$, a contour plot within this representation is given, keeping the total spin fixed, as well as the single-particle angular momentum. Notice the rather constant grow in magnitude of $\omega_2$ when the single-particle potential $V$ increases. 
\begin{figure}
    \centering
...
Concerning the canonicity factors, they are also graphically represented separately for $k_1$ and for $k_2$. Fig. \ref{fig-k-1-2-factors} shows the evolution of both quantities with respect to the total angular momentum. The representation used arbitrary values for the inertia factors, $V$, $\gamma$ and $j$. Note that just for a pedagogical purpose, the plots contain the regions where $I\leq j$, which is not the case when studying wobbling bands of isotopes.
\begin{figure}
    \centering
