...
With the help of Eq. \ref{lu163-absolute-energies-tsd1234}, a compact analytical structure for the wobbling spectrum of $^{163}$Lu is obtained within the $\mathbf{W_1}$ formalism. Additionally, having the base rule in calculating of the excitation energies (by subtraction of Eq. \ref{lu163-absolute-energy-band-head} from each level) will allow one to fix the primary step in the fitting procedure. As for the components that comprise the formula of the energy itself, the term $\mathcal{H}_\text{min}^I$ is adopted in line with  Eq. \ref{minimal-energy-term-hmin} and the wobbling frequencies $\Omega_{1,2}$ are taken from Eq. \ref{wobbling-frequencies-Omega-1-2}. 
