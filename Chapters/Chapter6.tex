...
The vector states $\ket{j\Omega}$ describe the general wave-function for a particle $\chi$ with the structure \cite{hecht1962asymmetric}:
\begin{align}
...
Besides the electric transition operator, the wave-functions for the wobbling bands considered in the current picture are also needed. The ground-bands are attained from the variational principle (the trial function from Eqs. \ref{tdve-approach-w1} - \ref{trial-function-appeoach-w1}):
\begin{align}
