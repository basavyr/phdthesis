...
Regarding the wave-functions used for the numerical calculations in the $\mathbf{W_1}$ picture, these can be constructed from the expansion up to first-order (abbreviated `1st or.') in the coordinates $(r,\varphi;f,\psi)$ and then perform re-quantization of the coordinates. Thus, from the wave-function of Eq. \ref{wave-function-TSD1-p0} one obtains:
\begin{align}
...
% improve phrasing for "from" terms

The wave-function $\ket{\Psi_{IMj}^{(1)}}$ can be applied for the transitions 

from zero-phonon bands, which comprise the bands TSD1-2 in $^{161,163,165,167}$Lu and TSD4 in $^{163}$Lu. However, transitions in and out of a one-phonon wobbling band (such as TSD3 

from $^{163}$Lu and $^{165}$Lu) require a wave-function built as an excitation of its corresponding ground-band partner. In the $\mathbf{W_1}$ picture, this can be inferred 

from the renormalization specified in Eqs. \ref{tsd-bands-compressed-spectrum} - \ref{tsd-bands-general-spectrum}. Hence, a state $I$ 

from the one-phonon band is obtained with the activation of the $I-1$ state 

from the neighboring zero-phonon partner with $n_{w_1}$ phonon operators $\Gamma_1^\dagger$ and $n_{w_2}$ phonon operators $\Gamma_2^\dagger$, each applied on the vacuum states $|\varnothing)_{I-1}$ and $|\varnothing)_j$:
\begin{align}
...
\end{align}
where the two vacuum states are defined in Appendix D 

from Ref. \cite{raduta2017semiclassical}.

% From the considerations made in Eqs. \ref{tsd-bands-compressed-spectrum} - \ref{tsd-bands-general-spectrum}, the spectrum depicted in Eq. \ref{lu163-absolute-energies-tsd1234}, and the collection of wobbling phonon numbers provided in Tables \ref{lu-163-phonon-numbers} - \ref{lu-167-experimental-data-table}, 
% % re-do phrasing
% it occurs that \emph{only the first phonon $\Gamma_1^\dagger$ (corresponding to the motion of the core) will generate excited phonon states}. 

% This results in

% the straightforward structure of the bands TSD3 from $^{163,165}$Lu:
% \begin{align}
