...
\end{align}
where the \emph{phonon} term $\mathcal{F}_{n_{w_1}n_{w_2}}^I$ is the sum of the two solutions:
\begin{align}
    \mathcal{F}_{n_{w_1}n_{w_2}}^I=\hbar\Omega_1\left(n_{w_1}+\frac{1}{2}\right)+\hbar\Omega_2\left(n_{w_2}+\frac{1}{2}\right)\ ,
\end{align}
which is obtained from the procedure of linearization described above. The conditions for which $\Omega_{1,2}$ exist and meet the criteria of positiveness were also summarized in Table \ref{omega-positive-conditions}. Throughout the calculations, the ordering $\Omega_1>\Omega_2$ was consistently adopted.