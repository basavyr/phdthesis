...
The situation $C=0$ imposes that the terms (at least one) inside the curly brackets from Eq. \ref{omega-C-term} will equate to zero. However, for simplifying the calculations, some notations should be introduced. Firstly, from the three inertia factors a certain feature can be exploited: the factors $A_k$ are defined as $A_k=1/(2\mathcal{I}_k)$. Additionally, the moments of inertia (both in rigid representation as well as the irrotational ones) are typically expressed in terms of $\beta_2$, $\gamma$ and a common scaling factor (recall Eq. \ref{eq-irrotational-rigid-mois}):
\begin{align}
    \mathcal{I}_k=\mathcal{I}_0\cdot h(\beta_2,\gamma;k)\ ,
\end{align}
where $h$ is defining the MOI either in the rigid picture or in the irrotational picture. Similarly for $A_k$ one has:
\begin{align}
...
\end{align}
Now it can be seen why such a transformation for $A_k$ is useful: the constant $1/\mathcal{I}_0$ can be factored out from $B$ and $C$, leaving only one `independent variable' $S=\mathcal{I}_0V$. Getting back to $C=0$, after some rearrangement the following is obtained:
\begin{align}
