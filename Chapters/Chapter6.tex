...
For calculation of the transition quadrupole moment $Q_I$ (Eq. \ref{quadrupole-transition-moment-QI}) and the SMQ (Eq. \ref{static-quadrupole-moment}), the results are compared with the experimental data in Table \ref{quadrupole-QI-QSQM-numerical-results}. The numerical results for $Q_\text{SQM}$ are evaluated by fixing $K=1/2$ and then applying the formula. 
\begin{table}
    \centering
    \resizebox{\textwidth}{!}{%
...
The interband transitions $I\to I-1$ were evaluated for the isotopes and the comparison between the theoretical results and the experimental data is provided in Table \ref{interband-E2-numerical-results}.
\begin{table}
    \centering
    ...
    \label{interband-E2-numerical-results}
\end{table}

Concerning the static quadrupole moments presented in Table \ref{quadrupole-QI-QSQM-numerical-results}, by keeping the value of $K$ fixed across each state, then for sufficiently large spins will result in $Q_\text{SQM}(I)\approx Q_I/2$, which can be actually observed from the table. Such a behavior translates in the total a.m. becoming perpendicular to the $1$-axis. On the other hand, if $K=I$ any variation emerging from rotational motion becomes negligible and $Q_\text{SQM}(I)$ tends to $Q_I$ \cite{bohr1998nuclear}. A graphical representation for $Q_\text{SQM}(I)$ with fixed $K=1/2$ can be seen for $^{163}$Lu in Fig. \ref{static-quadrupole-fixed-K-TSD1} for TSD1 and Fig. \ref{static-quadrupole-fixed-K-TSD2} for TSD2. Since this is the only isotope that has actual experimental numbers, the representations for the other isotopes were ignored.
\begin{figure}
    \centering
    ...
    \caption{The static quadrupole moment from Eq. \ref{static-quadrupole-moment} graphically represented as a function of total spin $I$ for the first band in $^{163}$Lu. The data are evaluated by keeping $K$ fixed to $K=1/2$ in the \textbf{left} inset, while $K=I$ in the \textbf{right} inset. See text for more details.}
...
\begin{figure}
    ...
    \caption{The static quadrupole moment from Eq. \ref{static-quadrupole-moment} graphically represented as a function of total spin $I$ for the band TSD2 in $^{163}$Lu. The data are evaluated by keeping $K$ fixed to $K=1/2$ in the \textbf{left} inset, while $K=I$ in the \textbf{right} inset. See text for more details.}
