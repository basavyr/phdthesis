...
The coordinate set has some stationary points, which are given by the vanishing time derivatives of the canonical variables. These stationary points are obtained from the critical conditions associated to the classical energy function:
\begin{align}
    \left(\frac{\partial \mathcal{H}}{\partial r}\right)=0\ ,\ \left(\frac{\partial \mathcal{H}}{\partial \varphi}\right)=0
\end{align}
for the the first set, and:
\begin{align}
    \left(\frac{\partial \mathcal{H}}{\partial f}\right)=0\ ,\ \left(\frac{\partial \mathcal{H}}{\partial \psi}\right)=0
\end{align}
for the second set. Evaluating the critical points of $\mathcal{H}$, it is possible to obtain the points at which $\mathcal{H}$ is \emph{minimal} (given by the sign of the Hessian). In order to meet such a criteria for $\mathcal{H}$, one needs to set an ordering of the three inertia factors. Choosing the largest MOI to be around the $3$-axis and $\mathcal{I}_3>\mathcal{I}_2>\mathcal{I}_1$ (or, equivalently $A_1<A_2<A_3$), the function achieves a minimum value at $p_0=(I,0;j,0)$:
\begin{align}
    \mathcal{H}|_\text{min}=\mathcal{H}(I,0;j,0)\ .
\end{align}