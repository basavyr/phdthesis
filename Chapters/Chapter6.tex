...
In the numerical calculations the set of formulas obtained in Eq. \ref{lu163-absolute-energies-tsd1234} will be all subtracted from the band-head energy, obtaining thus the \emph{excitation energies} (recall Eq. \ref{excitation-energy-general-formula}) for $^{163}$Lu. The band-head energy $E_{I_b,0,0}^\text{TSD1}$ is energy corresponding to the spin state $I_b=13/2$ from TSD1. Its expression is as follows:
\begin{align}
...
Putting together the set of energies from Eq. \ref{lu163-absolute-energies-tsd1234}, one can define a general formula that will illustrate the analytical formalism for the excitation energy used in the fitting procedure:
\begin{align}
    {\color{blue}E_{I_b}^\text{TSD1}(^A\text{Lu\ ;\ abs})}=&{\color{blue}\epsilon_{13/2}+\mathcal{H}_\text{min}^{I_b}+\mathcal{F}_{00}^{I_b}}\ ,\nonumber\\
    E_{I,n_{w_1},n_{w_2}}^\text{TSDK}(^A\text{Lu\ ;\ exc})=&{\color{red}\left(\epsilon_{j}+\mathcal{H}_\text{min}^I+\mathcal{F}^I_{n_{w_1}n_{w_1}}\right)}-{\color{blue}E_{I_b}^\text{TSD1}(^A\text{Lu\ ;\ abs})}\ ,
    \label{general-excitation-energy-fitting-model}
\end{align}
where $^A$Lu represents the isotope that is evaluated (i.e., $A\in\{161,163,165,167\}$), $I$ is a state belonging to a triaxial band $K$ ($I\in\text{TSDK}$) from $^A$Lu, the labels `$\text{exc}$/$\text{abs}$' symbolize the excitation/absolute energy, respectively, and $I_b$ is the band-head from the TSD1 band of that particular nucleus, which is given explicitly in the last column of Tables \ref{lu-161-experimental-data-table} - \ref{lu-167-experimental-data-table}. The coloring of Eq. \ref{general-excitation-energy-fitting-model} aims at depicting the absolute value that was obtained through TDVE (i.e., Eq. \ref{tsd-bands-general-spectrum}) indicated by red color, and the absolute value of the band-head marked by the blue color. Note that regarding $E_{I_b}^\text{TSD1}(^A\text{Lu\ ;\ abs}$, the subscripts $(n_{w_1},n_{w_2})$ have been dropped entirely since all ground-bands are zero-phonon bands and moreover, the quasi-particle involved in the particle + rotor coupling for the ground bands is $\mathcal{Q}_1=\pi(j_{13/2})$ across entire mass region.