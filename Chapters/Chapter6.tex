...
\begin{align}
    E_I^\text{TSDK}(^A\text{Lu\ ;\ exc})=\left(\epsilon_{j}+\mathcal{H}_\text{min}^I+F^I_{n_{w_1}n_{w_1}}\right)-E_{I_b}^\text{TSD1}(^A\text{Lu\ ;\ abs})\ ,
\end{align}
where $A$ represents the isotope that is evaluated (i.e., $A\in\{161,163,165,167\}$), $I$ is a state belonging to a triaxial band $K$ ($I\in\text{TSDK}$) from $^{A}$Lu, the labels `$\text{exc}$/$\text{abs}$' symbolize the excitation or the absolute energy, respectively, and $I_b$ is the band-head from the TSD1 band of a corresponding isotope, which is given explicitly for each isotope in Tables \ref{lu-161-experimental-data-table} - \ref{lu-167-experimental-data-table}.