...
The quadrupole transition probabilities can be determined from the matrix elements of the \emph{quadrupole transition operators}, which is consistent with the definition given in Eq. \ref{eq-general-reduced-E2}. Therefore, one needs the quadrupole transition operator prior to any calculations. 
...
\begin{align}
    \mathcal{M}(E2,\mu)=z_\text{eff}\left[\mathcal{D}_{\mu 0}^2Q_{0}-\left(\mathcal{D}_{\mu 2}^2+\mathcal{D}_{\mu -2}^2\right)Q_{2}\right]+z_\text{eff}\sum_{\nu=-2,0,2}^2 \mathcal{D}_{\mu\nu}^2 r^2Y_2^\nu\ .
