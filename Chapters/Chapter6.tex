...
\begin{align}
    ...
    \label{renormalized-bands-structure}
\end{align}
where the two protons (quasi-particles of particle character $\mathcal{Q}_p$) are denoted with $\mathcal{Q}_1$ and $\mathcal{Q}_2$ and their angular momenta will be labelled $j_1=13/2$ for $\mathcal{Q}_1$ and $j_2=9/2$ for $\mathcal{Q}_2$. The `p' abbreviation from the quasi-particle notation (as per the rules in Table \ref{notation-table-wobbling}) will be dismissed from now on because all the discussed isotopes have odd quasi-particles of \emph{particle} character.