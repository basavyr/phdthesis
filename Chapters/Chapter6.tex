...
Usually, the dynamic MOI is sensitive to the single-particle effects such as alignment of nucleons or even interaction between states belonging to different bands. As such, it could be an useful indicator of triaxiality and wobbling motion, since the nucleus can suffer structural changes as its angular momentum (and implicitly rotational frequency) increases. Looking at the figures with $\mathcal{I}^{(2)}$ for each isotope, some interesting features appear. Firstly, regarding the theoretical calculations, $\mathbf{W_1}$ formalism predicts constant values for this quantity, meaning that the dynamic MOI is a constant function of $\hbar\omega$ and furthermore the rotational frequency is linear in $I$. This can be indeed observed throughout the plots, where each TSD band has a constant $\mathcal{I}^{(2)}$ for the entire range of $\hbar\omega$. Nonetheless, if one `averages out' the experimental set of points to a singular value for each band, the theoretical and experimental lines would lie quite close to each other. On the other side, for the experimental data there is a staggering present (e.g., band TSD1 from $^{161}$Lu) and even sharp changes in magnitude, as it is the case for TSD2 from $^{161}$Lu and $^{165}$Lu, respectively. In $^{161}$Lu for example, the abrupt increase is caused by the alignment of the odd-proton's a.m., showing up at $\hbar\omega=0.45\ \text{MeV}$. The staggering behavior in the triaxial bands of the isotopes, mostly occurring in the low $\hbar\omega$ region, is caused by interaction between the states from the TSD bands and their neighboring normally deformed structures. Remarkable the fact that TSD1 from $^{165}$Lu exhibits a rather constant dynamic MOI (just as the  theoretical value) except for the the first two states. Comparing the present calculations with the results from Ref. \cite{raduta2018wobbling}, an overall improvement in the agreement with the experimental data can be observed.

\subsection{Electromagnetic Transitions}

The electromagnetic transitions between states were also calculated with the $\mathbf{W_1}$ formalism and compared with the available experimental data. For collective phenomena, one usually expects enhanced transitions of quadrupole character and also large quadrupole moments. In fact, as it was discussed in Chapters \ref{chapter4} and \ref{chapter-5}, the large quadrupole moments and large mixing ratios $\delta$ comprise crucial `tests' for wobbling behavior. Regarding the types of transitions, the intraband and interband transitions are of interest. The wobbling bands having collective nature, the spin difference within a band is $\Delta I=2$ units of angular momentum, while two states from adjacent bands only differ by one unit $\Delta I=1$ (recall Fig. \ref{wobbling-geometry-tilting-sketch} from discussion in Chapter \ref{chapter-5}). A schematic representation that illustrates the difference between interband/intraband transitions can be seen in Fig. \ref{schematic-interband-intraband-E2}. Therein, the E2 transitions are sketched for two states belonging to same band and two states from different bands and for each band, the allowed spin sequences are exemplified. For collective states, an electromagnetic transition of magnetic character can only occur between states that differ in one unit of a.m., meaning that a magnetic dipole M1 transition is possible between of neighboring bands. Moreover, they will compete with the interband E2 transitions and this behavior is reflected in the mixing ratio.
\begin{figure}
    \centering
    \includegraphics[scale=1.1]{Chapters/Figures/transitions-wobbling-states.pdf}
    \caption{Schematic representation with the transitions that can occur in wobbling spectra of nuclei. The \emph{intraband} values ($E2_\text{in}$ ; blue) represent the transitions between a $I$ initial state and a $I-2$ final state belonging to the same band, which are characterized by $\Delta I=I_\text{initial}-I_\text{final}=2\hbar$. On the other hand, \emph{interband} values ($E2_\text{out}$ ; red) occur between states belonging to two adjacent bands and they are characterized by $\Delta I=I_\text{initial}-I_\text{final}=1\hbar$. The spin sequences for each band are given below the level schemes in curly brackets.}
    \label{schematic-interband-intraband-E2}
\end{figure}

\subsubsection{E2 Transitions}

The quadrupole transition probabilities can be determined from the matrix elements of the \emph{quadrupole transition operators}, which is consistent with the definition given in Eq. \ref{eq-general-reduced-E2}. Therefore, one needs the quadrupole transition operator prior to any calculations. The most general form of an electric transition operator defined with arbitrary multi-polarity $\lambda$ is usually expressed in terms of the continuous charge distribution \cite{heyde1994nuclear}:
\begin{align}
    M(E\lambda,\mu)=\int\rho(\mathbf{r})r^\lambda Y_\lambda^\mu\text{d}\mathbf{r}\ ,
\end{align}
where the charge density $\rho$ is typically considered for a system composed of $A$ point charges:
\begin{align}
    \rho(\mathbf{r})=\sum_i^A e_\text{eff}^i\delta\left(\mathbf{r}-\mathbf{r}_i\right)\ .
\end{align}
...
\begin{align}
    T_{2\mu}^\text{coll}=\mathcal{D}_{\mu 0}^2Q_{0}-\left(\mathcal{D}_{\mu 2}^2+\mathcal{D}_{\mu -2}^2\right)Q_{2}\ ,
...
Putting together the two terms from Eqs. \ref{electric-quadrupole-T-coll} and \ref{electric-quadrupole-T-sp}, the total quadrupole transition operator (Eq. \ref{quadrupole-transition-operator-terms}) can be properly depicted \cite{raduta2020new,raduta2020towards}:
\begin{align}
    \mathcal{M}(E2,\mu)=\left[\mathcal{D}_{\mu 0}^2Q_{0}-\left(\mathcal{D}_{\mu 2}^2+\mathcal{D}_{\mu -2}^2\right)Q_{2}\right]+z_\text{eff}\sum_{\nu=-2,0,2}^2 \mathcal{D}_{\mu\nu}^2 r^2Y_2^\nu\ .
    \label{electric-quadrupole-transition-operator}
\end{align} 
...
The complex variables $(z;s)$ correspond to the classical coordinates $(r,\varphi;f,\psi)$ as per the transformations employed in Section \ref{equations-of-motion-section} (see Eqs. \ref{z-s-variables}, \ref{changed-rho-sigma-variables} and \ref{eq-of-motion-approach-w1}). Keep in mind that the set $(r,\varphi;f,\psi)$ brought the equations of motion to a canonical form and for each classical coordinate the expressions were explicitly specified in Eq. \ref{eq-of-motion-explicit-coordinates}. It was determined that the CEF (Eq. \ref{full-classical-energy-function}) is minimal in the point $p_0$, which was given Eq. \ref{cef-minimum-point-p0}. Evaluating the wave-function from Eq. \ref{wave-function-TSD1} around this minimum point, the expression of $\ket{\Psi_{IMj}}$ becomes:
\begin{align}
...
\end{align}
where for the boson creation and annihilation operators $(a^\dagger,a;b^\dagger,b)$ there corresponds the vacuum state $\ket{\varnothing}_I$. Any arbitrary state $\ket{\Psi_{IMj}}$ can be spanned from the vacuum via the relation \cite{raduta2017semiclassical}:
\begin{align}
    \ket{m,n}_I=\frac{1}{\sqrt{m!n!}}\left(a^\dagger\right)^m\left(b^\dagger\right)^n\ket{\varnothing}_I\ .
\end{align}

Recall that each set of creation/annihilation operators $(a^\dagger,a;b^\dagger,b)$ corresponds to the classical coordinates $(r,\varphi;f,\psi)$ as per the re-quantization applied in Eqs. \ref{canonical-coordinates-quantized} - \ref{canonical-transformations-ab-qp}. The wave-functions which need to be used in the calculus of transition probabilities can be developed from the the following exploit: expansion up to first-order in the coordinates $(r,\varphi;f,\psi)$ and then perform re-quantization of the coordinates as per the mentioned recipe. Thus, from the wave-function of Eq. \ref{wave-function-TSD1-p0} one obtains:
\begin{align}
    \ket{\Psi_{IMj}^{(1)}}=&\frac{1}{2^{I+j}}\sum_{K\Omega}\binom{2I}{I-K}^{1/2}\binom{2j}{j-\Omega}^{1/2}\ket{IMK}\ket{j\Omega}\times\nonumber\\
...
Looking at Eq. \ref{wave-function-TSD1-p0-first-order}, indeed it can be seen that the first-order expansion is reflected through the presence of $a^\dagger$ and $b^\dagger$ as independent variables (that is, no mixed or quadratic terms). The canonicity factors $k_1,k_2$ defined in Eq. \ref{canonicity-factors} also show up for each coordinate. Lastly, $K,\Omega$ are the projections of the total and single-particle angular momentum vectors in the intrinsic coordinate system, respectively. The wave-function $\ket{\Psi_{IMj}^{(1)}}$ can be applied for the transitions concerning bands TSD1-2 and TSD4 in all the isotopes. However, transitions in and out of TSD3 from $^{163}$Lu and $^{165}$Lu require the one-wobbling-phonon band description. 

Considering the most general one-phonon band which emerges from the $\mathbf{W_1}$ picture, this can be inferred from the set of Eqs. \ref{tsd-bands-compressed-spectrum} - \ref{tsd-bands-general-spectrum}. Hence, a one-phonon band is obtained by activating states $I-1$ from the neighboring zero-phonon partner with $n_{w_1}$ phonon operators $\Gamma_1^\dagger$ and $n_{w_2}$ phonon operators $\Gamma_2^\dagger$, each applied on the vacuum states $\ket{\varnothing}_I$ and $\ket{\varnothing}_j$:
\begin{align}
    \ket{\Psi_{I+1Mj}}=\ket{\Psi_{IMj}^{(1)}}\frac{1}{\sqrt{n_{w_1}!}}\left\{\left(\Gamma_1^\dagger\right)^{n_{w_1}}\ket{\varnothing}_I\otimes\left(\Gamma_2^\dagger\right)^{n_{w_2}}\ket{\varnothing}_j\right\}\ ,
    \label{wave-function-TSD-excited-phonon-state}
\end{align}
while in the present model it occurs that only the first phonon (corresponding to the motion of the core) will generate excited phonon states and according to Tables \ref{lu-163-phonon-numbers} and \ref{lu-165-experimental-data-table}, the excited band TSD3 will have the simplified form:
\begin{align}
    \ket{\Psi_{I+1Mj}^\text{TSD3}}=\ket{\Psi_{IMj}^{(1)}}\left(\Gamma_1^\dagger\right)^{n_{w_1}}\ket{\varnothing}_I
    \label{wave-function-TSD3}
\end{align}


% From the calculated parameters $\mathcal{P}_\text{fit}$ shown in Table \ref{numerical-fitting-parameters-Lu-isotopes} it was concluded that the maximal MOI is $\mathcal{I}_1$ for each isotope. Consequently, $Q_{0}$ and $Q_{2}$ are the quadrupole components that give the degree of elongation and asymmetry with respect to this axis. Because the formalism $\mathbf{W_1}$ adopts a set of MOI which are determined freely from Eq. \ref{chi-2-fitting-function}, one cannot infer their actual nature (i.e., hydrodynamical or rigid). Additionally, the textbook example of quadrupole components from Eq. \ref{quadrupole-components-Q0-Q2} emerge from hydrodynamic considerations. Despite this, the two quadrupole moments should also considered as free quantities in order to be consistent with the three MOI. Consequently, the analytical expressions from Eq. \ref{quadrupole-components-Q0-Q2} cannot be used explicitly within the numerical evaluations. They are determined by fixing two intraband transitions for TSD21 and TSD2. 

% With this method, the values $Q_0=18.43\ \text{eb}$ and $Q_2=19.81\ \text{eb}$ are obtained for $^{163}$Lu. The results for the other isotopes are graphically represented in Fig. \ref{quadrupole-moments-fit-numerical-results}. From this figure, the change of ordering between the two quadrupole components can be observed at $^{165}$Lu, where also the magnitude drops quite low compared to the neighbors $^{163,167}$Lu. Remarking the fact that the isotope $^{167}$Lu has a set of components which are almost identical.
% \begin{figure}
%     \centering
%     \includegraphics[scale=0.65]{Chapters/Figures/Lu-exp-energies/fig19.pdf}
%     \caption{The calculated quadrupole moments $Q_0$ and $Q_2$, which enter in Eq. \ref{electric-quadrupole-transition-operator}. The unit for $Q$ is $\text{eb}$ and the mass number $A$ corresponds to each nucleus.}
