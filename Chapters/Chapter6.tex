...
\begin{table}
    ...
    \caption{The fitting parameters, i.e., the three moments of inertia, the single-particle potential strength, and the triaxiality $\gamma$ for each Lu isotope. The numerical results were obtained from the procedure described in text. The number of wobbling states and the obtained root-mean-square error are also given in the table.}
    \label{numerical-fitting-parameters-Lu-isotopes}
\end{table}

To evidentiate the dependence of the three MOI with respect to the atomic mass $A$, the numerical values of $\mathcal{I}_k$ from Table \ref{numerical-fitting-parameters-Lu-isotopes} are graphically represented in Fig. \ref{MOIs-A-behavior-Lu-isotopes}, where one remarks a change in their ordering at $A=163$. Indeed, such a change can be regarded as a \emph{phase transition}. Moreover, there is a similar ordering for $^{161}$Lu and the band TSD4 from $^{163}$Lu. As for the other bands, the $\mathcal{I}_2$ and $\mathcal{I}_3$ ordering specific for $^{161}$Lu is changed for the other nuclides.
\begin{figure}
    \centering
    \includegraphics[scale=0.65]{Chapters/Figures/fig1_moivalues.pdf}
    \caption{The fitted MOI plotted as a function of the mass number $A$ with the numerical values taken from Table \ref{numerical-fitting-parameters-Lu-isotopes}.}
    \label{MOIs-A-behavior-Lu-isotopes}
\end{figure}

From the obtained MOIs, one sees that the the maximal MOI is the one of the $1$-axis. As per the discussion regarding wobbling regime (that is longitudinal vs. transverse) it cannot be inferred through the current formalism if the system motion has in fact a longitudinal character. Even if a non-interacting core has a MOI ordering as $\mathcal{I}_2>\mathcal{I}_1>\mathcal{I}_3$, the particle + core interaction will renormalize the MOI, resulting in a strong increase of $\mathcal{I}_1$ (caused by the alignment) and a moderate decrease for $\mathcal{I}_2$ (caused by the pairing interaction). These simultaneous processes will bring the resulting nucleus into a state of motion with $\mathcal{I}_1$ as the dominant moment.