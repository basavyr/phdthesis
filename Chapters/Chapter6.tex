...
\end{align}
Note that the curly brackets from Eq. \ref{classical-energy-function-deviations} denote the three terms $\mathcal{H}_\mathscr{C}(r,\varphi)+\mathcal{H}_\mathcal{Q}(f,\psi)+\mathcal{H}_\text{mixed}(r,\varphi;f,\psi)$. As such, one can clearly see which are the terms with separate canonical variables and the mixed ones.

\textit{i) Non-coupling terms}

Ignoring the coupling terms from Eq. \ref{classical-energy-function-deviations}, the CEF will consist in the sum of two independent oscillators with the frequencies:
\begin{align}
...
In order to obtained real solutions for $\omega_1$, the following conditions for the three inertia factors must hold:
\begin{align}
...
\begin{align}
    A_3>&S'A_1-VT_1\ \text{and}\ A_2>S'A_1-VT_2\ ,\nonumber\\
    &\text{while:}\ A_3>A_2\ \text{or}\ A_3<A_2\ ,
\end{align}
with the terms $S$, $S'$, $T_1$, and $T_2$ defined as:
\begin{align}
    S&=\frac{2I-1-2j}{2I-1}\ ,\  S'=\frac{2j-2I-1}{2j-1}\ ,\nonumber\\
    T_1&=\frac{2j-1}{j(j+1)\cdot(2j-1)}\sqrt{3}\sin\left(\sqrt{3}\cos\gamma+\sin\gamma\right)\, \nonumber\\
    T_2&=\frac{2j-1}{j(j+1)\cdot(2j-1)}2\sqrt{3}\sin\gamma\ .
\end{align}
The positiveness for $V$ will make sure that the conditions given for $\omega_2$ are always satisfied.

\textit{ii) Coupling terms}

In order to treat the terms with $r'f'$ and $\varphi'\psi'$ from the expansion in Eq. \ref{classical-energy-function-deviations}, a \emph{quantization} should be employed on the canonical variables. Indeed, from the classical coordinates (obtained via the TDVE) of the phase space $S_\text{cls}$, one can also go back to a $S_\text{qt}$ space, where each coordinate will have a corresponding operator defined in that space. As such, the following transformations will be utilized \cite{raduta2020approach}:
\begin{align}
...
\end{align}
The notation of the operators is consistent with the fact that $(\varphi,\psi)$ represent the canonical coordinates and $(r,f)$ depict the canonical momenta. Furthermore, for each set of operators creation and annihilation operators are introduced as:
\begin{align}
...
\end{align}
while the factors $G_1$ and $G_2$ from Eq. \ref{P-Q-G1-G2-factors} will still keep the exact form. With the notations given in Eq. \ref{Sigma-Delta-factors}, the two oscillator frequencies and the canonicity factors achieve a more practical shape, namely:
\begin{align}
...
\end{align}
for the two canonicity factors. Notice the single-particle strength explicitly given for $\omega_2$ and also $k_2$.