...
From the calculated parameters $\mathcal{P}_\text{fit}$ shown in Table \ref{numerical-fitting-parameters-Lu-isotopes} it was concluded that the maximal MOI is $\mathcal{I}_1$ for each isotope. Consequently, the quadrupole components $Q_{0}$ and $Q_{2}$ give the degree of elongation and asymmetry with respect to this axis. Because the formalism $\mathbf{W_1}$ adopts a set of MOI which are determined freely from Eq. \ref{chi-2-fitting-function}, one cannot infer their actual nature (that is neither hydrodynamical nor rigid). Consequently, the two quadrupole moments should also be considered as free quantities in order to be consistent with the three MOI. Consequently, the analytical expressions from Eq. \ref{quadrupole-components-Q0-Q2} cannot be used explicitly within the numerical evaluations. In fact the two components are determined numerically per each isotope by fixing one intraband transition from TSD1 and one interband transition $\text{TSD2}\to\text{TSD1}$. With this method, the values $Q_0=18.43\ \text{eb}$ and $Q_2=19.81\ \text{eb}$ are obtained for $^{163}$Lu. The results for the other isotopes are graphically represented in Fig. \ref{quadrupole-moments-fit-numerical-results}. Looking at this figure, a change of ordering between the two quadrupole components can be observed at $^{165}$Lu, where also the magnitude of the components drops quite low compared to the neighboring nuclei. Remarking the fact that the isotope $^{167}$Lu has a set of components which are almost identical.
\begin{figure}
    \centering
