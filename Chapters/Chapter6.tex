...
\begin{align}
    \ket{\chi_\mathcal{Q}}&=\sum_{j\Omega}c_{j\Omega}\ket{\chi_{j\Omega}}=\sum_{j\Omega}c_{j\Omega}\ket{j\Omega}\nonumber\\
    \ket{\bar{\chi}_\mathcal{Q}}=&\sum_{j\Omega}c_{j-\Omega}\ket{\bar{\chi}_{j\Omega}}=\sum_{j\Omega}c_{j-\Omega}\ket{j-\Omega}=\sum_{j\Omega}(-1)^{j-1/2}c_{j\Omega}\ket{j-\Omega}\ .
...
\begin{align}
    ...
    &=\sqrt{\frac{2I+1}{16\pi^2}}\sum_K C_K \left[D_{MK}^I\ket{\chi}+(-1)^{I-1/2}D_{M-K}^I\ket{\bar{\chi}}\right]\ ,
...
These are the states from the quantal space, belonging to the Hamiltonian from Eq. \ref{total-ham-approach-w1}. Returning to the trial function $\ket{\Psi_{IjM}}$ defined as a coherent state with products of the form $\ket{IMK}$ (Eq. \ref{IMK-wigner-functions}) and $\ket{j\Omega}$ (Eq. \ref{j-Omega-single-particle-states}) (both comprised in Eq. \ref{total-wavefunction-oddA-general}), its expression must be further discussed in terms of $z$ and $s$.