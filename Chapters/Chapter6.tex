...
\end{align}
for TSD3. The two protons (quasi-particles of particle character $\mathcal{Q}_p$) are denoted with $\mathcal{Q}_1$ and $\mathcal{Q}_2$ and their angular momenta will be labelled $j_1=13/2$ for $\mathcal{Q}_1$ and $j_2=9/2$ for $\mathcal{Q}_2$. The `p' abbreviation from the quasi-particle notation (as per the rules in Table \ref{notation-table-wobbling}) will be dismissed from now on because all the discussed isotopes have odd quasi-particles of \emph{particle} character. The labelling for $\mathcal{Q}_1$ and $\mathcal{Q}_2$ are also given in Table \ref{lu-163-phonon-numbers}.

With the Eqs. \ref{renormalized-bands-structure-TSD124} - \ref{renormalized-bands-structure-TSD3} as `recipes', one can obtain the wobbling energies for this isotope as per Eq. \ref{tsd-bands-general-spectrum} by adopting the spin values $I$ and the wobbling phonon numbers $n_{w_1}$ and $n_{w_2}$ of each band in particular. These values are given in Table \ref{lu-163-phonon-numbers}. Notice that for TSD1-2-4, because they were obtained as ground states, the pair of phonon numbers are $(0,0)$. Moreover, the TSD3 states are activated only by the phonon number $n_{w_1}$, which is consistent with the theory from $\mathbf{W_0}$ where the excited wobbling states were generated by the phonon operator that corresponded to $\Omega_1$. As it will be seen, for all four bands the wobbling frequency $\Omega_2$ arises only as the zero-point energy with $n_{w_2}=0$ (that is $1/2\Omega_2^I$).
\begin{table}
    \centering
    \resizebox{\textwidth}{!}{%
    ...
    Bands & $n_{w_1}$ & $n_{w_2}$ & $\mathcal{F}_{00}^I$ & $I_t$    & $I_b$    & $\mathcal{Q}$    \\ \hline
    ...
    TSD3  & $1$       & $0$       & $\mathcal{F}_{10}^I=\frac{1}{2}\left(3\cdot\Omega_1^I+\Omega_2^I\right)$ & $33/2^+$ & $85/2^+$ & $j_1^\pi=13/2^+\stackrel{not}{\equiv}\mathcal{Q}_1$ \\ \hline
    ...
    \caption{The wobbling phonon numbers corresponding to the phonon frequencies $\Omega_1$ and $\Omega_2$, respectively (see Eq. \ref{phononic-term-tsd-energies}). For completeness the first and last states of each band are given, together with the a.m. of the quasi-particle $\mathcal{Q}$.}
    \label{lu-163-phonon-numbers}
\end{table}

Knowing how to compute the phonon terms according to Eq. \ref{phononic-term-tsd-energies} and by using the general formula obtained in Eq. \ref{tsd-bands-general-spectrum}, one can determine the wobbling spectrum of $^{163}$Lu through the following set of equations:
\begin{align}
...
\end{align}
where $(\epsilon_{13/2},\epsilon_{9/2})$ are the single-particle shell energies for $\mathcal{Q}_1$ and $\mathcal{Q}_2$, respectively.