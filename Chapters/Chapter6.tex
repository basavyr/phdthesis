...
The new \emph{renormalization} for $^{163}$Lu in $\mathbf{W_1}$ applies the variational principle (i.e., TDVE) for all states of both TSD1 and TSD2. Additionally, since the coupling scheme $\mathcal{Q}+\mathscr{C}$ is different for TSD4 (i.e., the proton with $j=9/2$ couples with the even-even core as opposed to the $13/2$ shell for the other bands) and the variational principle can be applied separately for TSD4 as well. As such, only the TSD3 states emerge as excited wobbling states, which are formed by the action of a phonon operator (similar to the one defined in $\mathbf{W_0}$) on TSD2.

All the states from TSD1 are produced by coupling the $j=13/2$ odd proton from $i$-orbital with a triaxial even-even core having the spin sequence $\mathscr{C}_1=0,2,4,\dots$, forming thus a zero-phonon wobbling band. On the other hand, states belonging to TSD2 are built by the same odd-particle but a different \emph{core}, namely $\mathscr{C}_2=1,3,5\dots$. Notice the even/odd dissimilarity between the two bands, which will lead to the conclusion (together with the alternating signature and large large signature splitting) that TSD1 and TSD2 are \emph{signature partner bands}. This thing will be discussed in more detailed in a separate section.