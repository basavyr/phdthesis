...
Some final remarks on the $\mathbf{W_1}$ formalism that was portrayed throughout the chapter are worth emphasizing before continuing with the next iteration of this semiclassical model. As it was mentioned at the beginning, $\mathbf{W_0}$ was the foundational part that lead to the development of a tool for accurately describing the energy spectra of wobbling nuclei. From the workflow of applying the VP (Eq. \ref{tdve-approach-w1}) on the ground-band TSD1 and then applying multiple phonon operators on these states to generate levels of TSD2,$\dots$ (recall Fig. \ref{phonon-operator-schematic}), one changed the methodology by introducing the renormalization of the bands (Eqs. \ref{renormalized-bands-structure-TSD124} - \ref{renormalized-bands-structure-TSD3}), i.e., $\mathbf{W_1}$. The trial function (Eq. \ref{trial-function-appeoach-w1}) is given as a product of two coherent states; one associated to the core and one to the odd proton. The initial model Hamiltonian (Eq. \ref{total-ham-approach-w1}) was dequantized and the Classical Energy Function emerged (Eq. \ref{full-classical-energy-function}). Performing a linearization procedure of the equations of motion (Eq. \ref{eq-of-motion-approach-w1}) around the minimum point of the CEF, two wobbling frequencies are obtained: $\Omega_1$ and $\Omega_2$ (defined in Eq. \ref{wobbling-frequencies-Omega-1-2}). The two frequencies define the motion of the core and the single particle, respectively. Having $\Omega_{1,2}$ and the minimal term $\mathcal{H}_\text{min}$ (Eq. \ref{minimal-energy-term-hmin}), the general spectrum of an odd-mass triaxial nucleus can be formed (Eqs. \ref{phononic-term-tsd-energies} - \ref{tsd-bands-general-spectrum}). Working with the excitation energies instead of the absolute energies in the current renormalization will bring this spectrum to a rather different structure (via the subtraction of the band-head energy) and this set of equations was used in the fitting procedure (recall Eq. \ref{general-excitation-energy-fitting-model} and diagram from Fig. \ref{fitting-workflow-fig}). Once the free parameters were obtained from the minimization of $\chi^2$-function (Eq. \ref{chi-2-fitting-function}), calculation of several quantities are performed and compared with experimental data. An overall good agreement was achieved across the mass region. Finally, calculations for the transition probabilities were performed using a wave-function for the $E2$ m.e. (i.e., $\ket{\Psi_{IM;j}}$) and a slightly modified one for $M1$ m.e. (recall $\ket{\Phi_{IM;j}}$). As expected, the model reproduces the measurements quite well.