...
\end{align}
where $A$ represents the isotope that is evaluated (i.e., $\{161,163,165,167\}$), $I$ is a state belonging to the triaxial band $K$ ($I\in TSDK$), the labels $\text{exc}$/$\text{abs}$ symbolize the excitation or the absolute energy, respectively, and $I_0$ is the band-head from the TSD1 band of a corresponding isotope, which is given explicitly for each isotope in Tables \ref{lu-161-experimental-data-table} - \ref{lu-167-experimental-data-table}.