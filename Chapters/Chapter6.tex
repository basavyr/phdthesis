...
Concerning the static quadrupole moments presented in Table \ref{quadrupole-moments-fit-numerical-results}, by keeping the value of $K$ fixed across each state, then for sufficiently large spins will result in $Q_\text{SQM}(I)\approx Q_I/2$, which can be actually observed from the table. Such a behavior translates in the total a.m. becoming perpendicular to the $1$-axis. On the other hand, if $K=I$ any variation emerging from rotational motion becomes negligible and $Q_\text{SQM}(I)$ tends to $Q_I$ \cite{bohr1998nuclear}. A graphical representation for $Q_\text{SQM}(I)$ with the two methods (i.e., fixed $K=1/2$ as in Table \ref{quadrupole-moments-fit-numerical-results} and $K=I$) for all isotopes can be seen in Fig. \ref{static-quadrupole-fixed-K-nonfixed-K}.
\begin{figure}
    \centering
    \caption{The static quadrupole moment from Eq. \ref{static-quadrupole-moment} graphically represented as a function of total spin $I$ for $^{161,163,165,167}$Lu. The red lines are evaluated by keeping $K$ fixed to $K=1/2$, while the black lines are given by evaluating $C_{K0K}^{I2I}$ with $K=I$. See text for more details.}
    \label{static-quadrupole-fixed-K-nonfixed-K}
\end{figure}
