...
\begin{align}
    E_{I_b}^\text{TSD1}(^A\text{Lu\ ;\ abs})=&\epsilon_{13/2}+\mathcal{H}_\text{min}^{I_b}+F_{00}^{I_b}\ ,\nonumber\\
    E_{I,n_{w_1},n_{w_2}}^\text{TSDK}(^A\text{Lu\ ;\ exc})=&{\color{red}\left(\epsilon_{j}+\mathcal{H}_\text{min}^I+F^I_{n_{w_1}n_{w_1}}\right)}-{\color{blue}E_{I_b}^\text{TSD1}(^A\text{Lu\ ;\ abs})}\ ,
