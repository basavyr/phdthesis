...
\end{align}
is \emph{minimal}, where $N_T$ represents the total number of states for the isotope. In the case of $^{163}$Lu, the $\chi^2$-function will contain all states belonging to TSD1-2-3 and TSD4. As an observation, the band-head energy $E_{13/2,0,0}^\text{TSD1}$ will be excluded because its corresponding excitation energy is null. Obviously, $E_\text{exp}^{(i)}$ represents \emph{experimental excitation energy}, which is evaluated by subtracting the band-head level $1.738\ \text{MeV}$ from each state. From a computational standpoint, the problem thereby consists of a minimization process that has to be tackled on the $\chi^2$-function.
